...
class GMPETable(GMPE):
    ...
    def _return_tables(self, mag, imt, val_type):
        ...
        if str(imt).startswith('PGA'):
            ...
            low_period = round(periods[0], 4)
            high_period = round(periods[-1], 4)
            low_period = round(min(periods), 4)
            high_period = round(max(periods), 4)
            #print(low_period, high_period)

            if round(imt.period, 4) < low_period or round(imt.period, 4) > high_period:
